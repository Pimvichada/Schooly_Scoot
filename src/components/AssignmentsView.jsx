        ...
        <div className={`space-y-6 ${darkMode ? 'text-slate-100' : ''}`}>
            ...
            {isAssignmentsLoading ? (
                <div className="space-y-4">
                    {[1, 2, 3, 4].map((i) => (
                        <div key={i} className={`p-6 rounded-3xl border ${darkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-100'} animate-pulse`}>
                            <div className="flex items-center gap-4">
                                <div className={`w-12 h-12 rounded-2xl ${darkMode ? 'bg-slate-800/50' : 'bg-slate-100'}`}></div>
                                <div className="flex-1 space-y-3">
                                    <div className={`h-4 w-1/4 rounded-lg ${darkMode ? 'bg-slate-800/50' : 'bg-slate-100'}`}></div>
                                    <div className={`h-3 w-1/3 rounded-lg ${darkMode ? 'bg-slate-800/50' : 'bg-slate-100'}`}></div>
                                </div>
                                <div className={`h-10 w-24 rounded-2xl ${darkMode ? 'bg-slate-800/50' : 'bg-slate-100'}`}></div>
                            </div>
                        </div>
                    ))}
                </div>
