...
export default function SchoolyScootLMS() {
  ...
  const [assignmentFiles, setAssignmentFiles] = useState({});  // Store files per assignment: { assignmentId: [files] }

  const [assignments, setAssignments] = useState(ASSIGNMENTS);  // Track assignment status

  const [selectedImageFile, setSelectedImageFile] = useState(null);  // For image preview modal
  ...
  const saveAssignmentFiles = () => {

    if (selectedAssignment && uploadFiles.length > 0) {

      setAssignmentFiles(prev => ({

        ...prev,

        [selectedAssignment.id]: [...(prev[selectedAssignment.id] || []), ...uploadFiles]
  ...
  const removeAssignmentFile = (assignmentId, fileIndex) => {
  ...
  const viewFileImage = (file) => {
    ...
    if (imageExtensions.includes(fileExtension)) {
      ...
      reader.onload = (e) => {
        ...
        setSelectedImageFile({ name: file.name, src: e.target.result });
      ...
      alert('ไฟล์นี้ไม่ใช่รูปภาพ หรือรูปแบบไม่รองรับ');
  ...
  const renderModal = () => {
    ...
    const handleSaveAssignment = () => {
      ...
      if (selectedAssignment && assignmentFiles[selectedAssignment.id]) {
          ...
          {activeModal === 'assignmentDetail' && selectedAssignment && (
            ...
            <div className="p-8">
              ...
              <div className="border-t border-slate-100 pt-6">
                ...
                {assignmentFiles[selectedAssignment.id] && assignmentFiles[selectedAssignment.id].length > 0 && (

                  <div className="mb-6">

                    <h4 className="font-bold text-slate-700 mb-3">ไฟล์ที่บันทึกแล้ว ({assignmentFiles[selectedAssignment.id].length})</h4>

                    <div className="space-y-2">

                      {assignmentFiles[selectedAssignment.id].map((file, index) => (

                        <div key={index} className="flex items-center justify-between bg-[#F0FDF4] p-3 rounded-lg border border-[#96C68E]">

                          <div className="flex items-center gap-3 flex-1 min-w-0">

                            <CheckCircle size={20} className="text-[#96C68E] flex-shrink-0" />

                            <div className="flex-1 min-w-0">

                              <p className="text-slate-700 font-medium truncate">{file.name}</p>

                              <p className="text-xs text-slate-500">{(file.size / 1024).toFixed(2)} KB • บันทึกแล้ว</p>
                          ...
                          <div className="flex gap-1 ml-2 flex-shrink-0">
                            ...
                            <button
                              ...
                              onClick={() => viewFileImage(file)}

                              className="p-2 text-blue-500 hover:bg-blue-100 rounded-lg hover:text-blue-700 transition-all"
                            ...
                            <button
                              ...
                              onClick={() => removeAssignmentFile(selectedAssignment.id, index)}

                              className="p-2 text-red-500 hover:bg-red-100 rounded-lg hover:text-red-700 transition-all"
                    ...
                    <div className="mt-4 pt-4 border-t border-slate-200">
                      ...
                      <p className="text-sm text-slate-600 mb-3">เพิ่มไฟล์เพิ่มเติม:</p>
                ...
                <div className="relative mb-4">
                  ...
                  <div className={`border-2 border-dashed rounded-2xl p-8 text-center transition-all ${uploadFiles.length > 0
                    ...
                    <p className="text-xs text-slate-400 mt-1">รองรับไฟล์หลายไฟล์</p>
                ...
                {uploadFiles.length > 0 && (
                  ...
                  <div className="mb-6">
                    ...
                    <h4 className="font-bold text-slate-700 mb-3">ไฟล์ใหม่ที่อัพโหลด ({uploadFiles.length})</h4>
                        ...
                        <div key={index} className="flex items-center justify-between bg-slate-50 p-3 rounded-lg border border-slate-200">
                          ...
                          <div className="flex gap-1 ml-2 flex-shrink-0">
                            ...
                            <button
                              ...
                              className="p-2 text-blue-500 hover:bg-blue-50 rounded-lg hover:text-blue-700"
                            ...
                            <button
                              ...
                              className="p-2 text-red-500 hover:bg-red-50 rounded-lg hover:text-red-700"
                ...
                <button
                  ...
                  disabled={uploadFiles.length === 0 && (!assignmentFiles[selectedAssignment.id] || assignmentFiles[selectedAssignment.id].length === 0)}

                  className={`w-full py-3 rounded-xl font-bold text-lg shadow-sm flex items-center justify-center transition-all ${(uploadFiles.length > 0 || (assignmentFiles[selectedAssignment.id] && assignmentFiles[selectedAssignment.id].length > 0))
                ...
                >
                  ...
                  <CheckCircle className="mr-2" /> {selectedAssignment.status === 'submitted' ? 'บันทึกการแก้ไข' : (uploadFiles.length > 0 ? 'บันทึกและส่งการบ้าน' : 'ส่งการบ้าน')}
    ...
    <div className="flex h-screen bg-[#F8FAFC] font-sans">
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden relative">
        ...
        {selectedImageFile && (
          ...
          <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4" onClick={() => setSelectedImageFile(null)}>

            <div className="bg-white rounded-2xl shadow-2xl max-w-4xl max-h-[90vh] flex flex-col" onClick={(e) => e.stopPropagation()}>

              <div className="flex justify-between items-center p-6 border-b border-slate-200">

                <h3 className="text-xl font-bold text-slate-800">{selectedImageFile.name}</h3>
              ...
              <div className="p-4 border-t border-slate-200 flex justify-center">
                ...
                <button
                  ...
                  className="px-6 py-2 bg-slate-200 text-slate-800 rounded-lg font-bold hover:bg-slate-300 transition-all"
